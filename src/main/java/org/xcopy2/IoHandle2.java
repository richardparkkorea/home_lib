...
public class IoHandle2 {
	...
	public IoHandle2(String id, int port, boolean overwrite) throws Exception {
		...
		try {
			...
			m_server = new MqttBroker3(port);

			// m_server.setCallBack(new IMqttBroker() {
			//
			// @Override
			...
			new Timer2().schedule(new Timer2Task() {
				...
				public void run(Timer2 tmr) {
					...
					// for(String s: m_server.getClientIds().keySet() ) {
					//
		...
		m_handle = new OneFileHandle2(id + "[" + port, id, port);// id meaning is IP Address.
	...
	public IoHandle2(String id, String server, int port, boolean overwrite) throws Exception {

		m_handle = new OneFileHandle2(id, server, port);// id meaning is IP Address.
		m_handle.m_overwrite = overwrite;
	...
	public OneFileHandle2 oh() {
		return m_handle;
	}

	// no server mode
	public IoHandle2() throws Exception {
	...
	public int[] XCopy(OneFileHandle2 rmi_s, String srcName, OneFileHandle2 rmi_d, String destName, boolean ignoreError,
			CopyingInterface2 cInter) {
		int copied = 0;
	...
	public int[] XCopy2(OneFileHandle2 rmi_s, String srcDir, String[] srcList, OneFileHandle2 rmi_d, String destDir,
			boolean ignoreError, CopyingInterface2 cInter) {
		int copied = 0;
	 ...
	 * 
	 */
	public boolean WriteFile(OneFileHandle2 sSvr, String src, OneFileHandle2 dSvr, String dest,
	 ...
	 * 
	 */
	public int[] XCopy(OneFileHandle2 rmi_s, String[] src, OneFileHandle2 rmi_d, String dest, boolean ignoreError,
			CopyingInterface2 cInter) {
		int copied = 0;
	...
	public int[] mirror(OneFileHandle2 rmi_s, String srcName, OneFileHandle2 rmi_d, String destName,
			boolean ignoreError, CopyingInterface2 cInter, String extFilter) {
		int copied = 0;
			...
			if (rmi_s.isFile(srcName) == true) {
				...
				for (String s : l) {
					...
					if (rmi_s.isDirectory(srcName + s)) {
						...
						System.out.format("ext check = %s  %s  %s  \r\n", extFilter(extFilter, srcName + s), extFilter,
								srcName + s);
