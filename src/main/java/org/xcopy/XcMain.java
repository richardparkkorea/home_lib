...
public class XcMain {
	...
	public static void start(String[] args) {
		...
		if (serverGuiMode) {
			...
			new frmServer(serverIp).setVisible(true);
			...
			try {
				...
				new IoHandle(serverIp, port, overwrite);
			...
			try {
				...
				if (ui) {
					frmCopying cf = new frmCopying(han, true);
				}
	 ...
	 * @throws Exception
	 */
	public static IoHandle server(String id, String ip, int port, boolean over) throws Exception {
		return new IoHandle(id, ip, port, over);
	}
	...
	public static int[] copy(String bridgeId, int port, String srcIp, String destIp, String srcDir, String[] src,
			String destDir, boolean ignoreErrors, boolean show, CopyingInterface cInter) throws Exception {

		IoHandle han = new IoHandle();
		//
		...
		// OneFileHandle ss = new OneFileHandle(null, srcIp, port);
		// OneFileHandle tt = new OneFileHandle(null, destIp, port);
		OneFileHandle ss = new OneFileHandle(null, bridgeId, port);
		OneFileHandle tt = new OneFileHandle(null, bridgeId, port);
		...
		// RmiClient rc = new RmiClient();
		if (show) {
			frmCopying cf = new frmCopying(han, false);
		}
	...
	public static Image createImage(String path, String description) {
		URL imageURL = XcMain.class.getResource(path);
	...
	public static ImageIcon getIcon(String s, int w, int h) {
		...
		try {
			URL imageURL = XcMain.class.getResource(s);
