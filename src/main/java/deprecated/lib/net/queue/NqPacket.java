 ...
 * @version 1.0
 */
final public class NqPacket implements INqPacketInterface {
	...
	public NqPacket(NqServer p, Socket s) {
		...
		synchronized (m_syncRecvLock) {
			...
			if (checkPacketData(m_receviedDataCollector.bufferHandle(), m_receviedDataCollector.length()) == -1) {
				...
				if (n > 0) {
					...
					//
					//
					NqEvent in_out_val = new NqEvent();

					// in_out_val.len = 0;
					in_out_val.msg = NqEvent.EventNone;
					in_out_val.sk = m_sock;
					in_out_val.IPacket = this;
					...
					if (packetReadBytes('s', bufData, bufLen, data_st) >= 0) {
						...
						in_out_val.msg = NqEvent.EventRecevied;
