...
import deprecated.lib.net.bridge.CTcpQueueListener;
 ...
 */
@Deprecated
public class RmQueue implements Runnable {

	// public final static long BASE_BUFFER_SIZE = 1024 * 1024*8;
	...
	private ConcurrentMap<SocketChannel, RmPacketPicker> m_channels;
	...
	public void setChannelAliveTimeout(long l) {
 	...
 	public RmQueue(int port) throws Exception {

		synchronized (m_lock) {
			this.port = port;

			m_channels = new ConcurrentHashMap<SocketChannel, RmPacketPicker>();
	...
	private void accept(SelectionKey key) throws IOException {
		...
		//
		synchronized (m_channels) {
			m_channels.put(channel, new RmPacketPicker(MAX_RECEIVABLE_SIZE));
		}
		channel.setOption(StandardSocketOptions.SO_SNDBUF, (int) SOCKET_BUFFER_SIZE);
		channel.setOption(StandardSocketOptions.SO_RCVBUF, (int) SOCKET_BUFFER_SIZE);
	...
	private void read(SelectionKey key, ByteBuffer buffer) throws IOException {
		...
		while ((rs = ep.check()) != null) {
			...
			try {
				...
				// 160422
				if (rs.to != null && rs.to.trim().length() == 0 && rs.from_pwd != null) {
					// System.out.println("echo~1");
					if (rs.from_pwd.equals("putId")) {
						rs.result = true;
						rs.to = rs.from;
						rs.privateCode = 'r';

						if (channelFromName.get(rs.from) != null)
							rs.setString("res", "update");
						else
							rs.setString("res", "add");
			...
			if (channelFromName.containsValue(channel) == false) {
				...
				if (m_actionListener.addClientId(rs.from, rs.from_pwd, channel)) {
					...
					try {
						...
						rs.to = rs.from;
						rs.privateCode = 'r';
						rs.remote_error = "rejected by server";
					} catch (Exception e) {
						e.printStackTrace();
					}
				}
	...
	private void internalSend(SocketChannel ch, CBundle rs) {
		...
		try {
			...
			send(ch, RmPacketPicker.make(rs));
