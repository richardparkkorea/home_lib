...
public class MqttClient implements TmsSelectorListener, SyncSocketListener {
	...
	// static
	//
	static Map<MqttClient, String> clientArr = new HashMap<MqttClient, String>();

	public static int add(MqttClient c) {
	...
	static String remove(MqttClient c) {
		synchronized (clientArr) {
			return clientArr.remove(c);
		} // sync
	}

	public static boolean isKeepConnection(MqttClient c) {
		synchronized (clientArr) {
	...
	// memebers
	//
	Object openCloseLock = new Object();

	boolean m_isClosed = false;
	...
	public MqttClient(MqttBroker broker, SocketChannel new_client) throws Exception {
	 ...
	 * @throws Exception
	 */
	public MqttClient connect(String ip, int port) throws Exception {
		m_isClosed = false;
		return _connect(ip, port, 60, null, null, 6);
	}

	@Deprecated
	public MqttClient connect(String ip, int port, int ka, String user, String pwd) throws Exception {
		m_isClosed = false;
	 ...
	 * @throws Exception
	 */
	public synchronized MqttClient _connect(String ip, int port, int ka, String user, String pwd, double waitConnact)
			throws Exception {
	 ...
	 * @return
	 */
	public MqttClient keepConnection() {
		return keepConnection(null,null);
	}

	public MqttClient keepConnection(String subTopic,IMqtt fun) {
		add(this);

		if (subTopic != null && subTopic.trim().length() > 0 && fun!=null) {
			
			m_callback = fun;
			Topic topic = new Topic(subTopic);
		...
		if (myTimer == null) {
			myTimer = new Timer2().schedule(new Timer2Task() {
				...
				public void run(Timer2 tmr) {
					...
					synchronized (clientArr) {
						arr = clientArr.keySet().toArray(new MqttClient[0]);
					} // sync
	...
	public MqttClient setId(String new_id) {
		m_clientId = new_id;
		return this;
	}
	...
	public synchronized MqttClient publish(String tstr, byte[] payload, int pay_length, int qos) throws Exception {
		...
		if (workInCallBack && qos != 0) {
			throw new UserException("only qos=0 support");
		}
	...
	public MqttClient publish(String t, String s, int qos) throws Exception {
	...
	public void recv(SyncSocket ask, byte[] buf, int len) {
		...
		debug(MyLogger.DEBUG, "client.recv=%s len=%s   msg.buf.len=%s ",
				StringUtil.omitAfter(StringUtil.ByteArrayToHex(rxd), 100), rxd.length, message.buffer.length());
