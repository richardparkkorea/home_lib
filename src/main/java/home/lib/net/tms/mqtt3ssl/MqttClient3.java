 ...
 * 
 */
public class MqttClient3 implements MqttServerSocketListener, MqttSocketListener {
	...
	MqttSocket m_channel = null; // connection to remote broker
	...
	private String m_caCrt = null;

	private int m_writeErrorCount = 0;
	...
	public MqttClient3(MqttBroker3 broker, MqttSocket new_client) throws Exception {
		...
		synchronized (m_openCloseLock) {
			...
			if (connectNow) {
				try {
					...
					new MqttSocket(ip, port, this, m_caCrt);
	 ...
	 * @throws Exception
	 */
	private void sendConact(MqttSocket ask) throws Exception {
		...
		if (myTimer == null) {
			myTimer = new Timer2().schedule(new Timer2Task() {
				...
				public void run(Timer2 tmr) {
					...
					// MqttBroker.debug(MqttBroker3.DEBUG, String.format("alive.tmr=%s/%s ",loop, arr.length) );
					...
					// debug(MqttBroker3.DEBUG, "do myLoop(id:%s)", arr[loop].getId());
	...
	int processMessage(MqttMessage in) throws Exception {
		...
		debug(MqttBroker3.DEBUG, "msg.len=" + mesg.buffer.copyOf().length + " mesg.buf= %s",
				StringUtil.omitAfter(StringUtil.ByteArrayToHex(mesg.buffer.copyOf()), 100));

		try {
			if (m_broker != null) {
				m_broker.increaseEventCount();
			}
		} catch (Exception e) {
		...
		case MqttMessage.Type_SubAck:// ::Type::SubAck:
			...
			// debug(MqttBroker3.VERBOSE, "msg.len=" + mesg.buffer.copyOf().length + " mesg.buf= %s",
			// StringUtil.omitAfter(StringUtil.ByteArrayToHex(mesg.buffer.copyOf()), 100));
	...
	public void selectorReceived(MqttSocket ch, byte[] rxd) {
		...
		for (int n = 0; n < rxd.length; n++) {
			...
			if (m_message.state == MqttMessage.State_Error) {
				...
				debug(MqttBroker3.WARNING, "MqttMessage.State_Error");
				try {
			...
			if (m_message.type() != 0) {
				...
				debug(MqttBroker3.DEBUG, "recv type= %x   len=%s   state=%s ", m_message.type(),
						m_message.buffer.length(), m_message.state);
	...
	public void received(MqttSocket ask, byte[] buf, int len) {
		...
		debug(MqttBroker3.DEBUG, "client.recv=%s len=%s   msg.buf.len=%s ",
		...
		for (int n = 0; n < rxd.length; n++) {
			...
			if (m_message.type() != 0) {
				...
				debug(MqttBroker3.DEBUG, "recv type= %x", m_message.type());
	...
	public synchronized MqttClient3 keepConnection(String ip, int port, int ka, String user, String pwd, IMqtt3 cb)
			throws Exception {
		m_isClosed = false;// @20240517 add

		try {
			// return _connect(ip, port, ka, user, pwd, cb, false);
			return _connect(ip, port, ka, user, pwd, cb, true);// @20250110
		} finally {
			addToMgr(this);// <-------------- start the connectivity maintenance feature// @20250110
		}
	...
	public MqttClient3 setCaCrt(String caCrt) {
		m_caCrt = caCrt;
		return this;
	}
