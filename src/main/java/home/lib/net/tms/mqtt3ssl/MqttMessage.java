...
class MqttMessage {
	...
	byte sendTo(MqttClient3 client)// MqttError sendTo(MqttClient*);
...
//
	...
	byte sendTo(MqttSocket sock)// MqttError sendTo(MqttClient*);

	{

		if (buffer.length() > 0 && sock != null) {

			encodeLength();

			byte[] b = buffer.copyOf();

//			MqttBroker3.debug(MyLogger.DEBUG,
//					" **send.len=" + b.length + " buf=" + StringUtil.omitAfter(StringUtil.ByteArrayToHex(b), 128));
