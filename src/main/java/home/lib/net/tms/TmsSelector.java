...
public class TmsSelector {
	...
	class UserData {
		...
		UserData(boolean accept, Object uo, double t) {
			isAccept = accept;
			userobj = uo;
			timeout = t;
		}
	...
	private ConcurrentMap<SocketChannel, UserData> m_channels = new ConcurrentHashMap<SocketChannel, UserData>();

	// private ConcurrentMap<SocketChannel, TimeUtil> m_channelsLastRecv = new ConcurrentHashMap<SocketChannel,
	// TimeUtil>();// 200407

	private ConcurrentMap<SocketChannel, UserData> m_connecting = new ConcurrentHashMap<SocketChannel, UserData>();
	...
	private TmsSelectorListener m_svr = null;
	...
	public TmsSelector(TmsSelectorListener svr) {
		// m_bootstrap = bs;
		m_svr = svr;
	}
	
	public TmsSelector bind(String ip, int port) throws Exception {
		return bind(ip,port,0);
	}

	public TmsSelector bind(final String ip, final int port, int backlog) throws Exception {
		...
		new Thread(new Runnable() {
			public void run() {
				...
				debug(MyLogger.DEBUG, "create selector cnt=%d. hashcode=%x ", m_total_selector_count, hashCode());
				try {
					m_alive2 = true;
					...
					e.printStackTrace();
					debug(MyLogger.DEBUG, "doSelectAction has erro!.(dl=%s) hashcode=%x", debugLocate, hashCode());
					debug(e);

				} finally {

					try {
						SocketChannel chs[] = getChannels();
					...
					debug(MyLogger.DEBUG, "doSelectAction is finished! port(%s) hashcode=%x", m_port, hashCode());
				...
				debug(MyLogger.DEBUG, "finish!!! (%s:%s) cnt=%d  ", ip, port, m_total_selector_count);
	...
	public void close() {
		...
		m_alive2 = false;
		debug(MyLogger.DEBUG, "-selector.close();  dl=%s ", debugLocate);
	...
	public Object getUserObject(SocketChannel ch) {
		synchronized (m_channels) {
						...
						// if (change != null)
						{
							...
							case NioChangeRequest.CHANGEOPS:
								try {
									...
									// e.printStackTrace();
									debug(MyLogger.VERBOSE, "close in CHANGEOPS (%s)", m_changeRequests.size());
							...
							case NioChangeRequest.REMOVE:
								...
								try {
									debug(MyLogger.VERBOSE, " MqChangeRequest.REMOVE");
...

						{
							try {
								if (!key.isValid()) {
									...
									debug(MyLogger.VERBOSE, "!key.isValid()");
	...
	//
	// synchronized (this.m_changeRequests) {
	// debug(MyLogger.VERBOSE, "add remove -3.1 ");
		...
		try {
			if (tmrCheckChannelAlive.end_sec() > 60) {// every 10 min
				...
				synchronized (m_channels) {
					...
					for (Map.Entry<SocketChannel, UserData> entry : m_channels.entrySet()) {
						// System.out.println("Key = " + entry.getKey() + ", Value = " + entry.getValue());
						...
						if (u.tmrLastRecv.end_sec() > u.timeout  && m_channelTimeoutSec!=0) {
							...
							synchronized (this.m_changeRequests) {
								debug(MyLogger.VERBOSE, "timeover( %s > %s ) ", u.tmrLastRecv.end_sec(), u.timeout);
								this.m_changeRequests
										.add(new NioChangeRequest(entry.getKey(), NioChangeRequest.REMOVE, 0));
	...
	private void onAccept(SelectionKey key) throws IOException {
		...
		try {
			...
			Object uo = m_svr.accepteded(channel);
			accept(channel, uo);
			...
			debug(MyLogger.DEBUG, "error in accept %s ", e);
			channelClose(channel);
		}
	 ...
	 * @throws IOException
	 */
	private void accept(SocketChannel channel, Object userobj) throws IOException {

		synchronized (m_channels) {

			m_channels.put(channel, new UserData(true, userobj, m_channelTimeoutSec));
	...
	private void read(SelectionKey key, ByteBuffer buffer) throws IOException {
		...
		try {
			...
			synchronized (m_changeRequests) {
				debug(MyLogger.VERBOSE, "read() err=" + e);
		...
		if (numRead < 0) {
			...
			synchronized (m_changeRequests) {
				debug(MyLogger.VERBOSE, "read() err len=" + numRead);
				this.m_changeRequests.add(new NioChangeRequest(channel, NioChangeRequest.REMOVE, 0));
			}
			return;
		}
	...
	public void channelClose(SocketChannel ch) {
		...
		synchronized (this.m_changeRequests) {
			debug(MyLogger.VERBOSE, "channelClose() ");
	...
	public boolean isAlive(SocketChannel channel) {
	...
	public boolean isAccept(SocketChannel channel) {
		...
		synchronized (this.m_pendingData) {
			...
			if (queue != null) {
				...
				try {
					...
					// e.printStackTrace();
					debug(MyLogger.VERBOSE, "close in write " + e.toString());
					// queue.clear();

					// removeChannel(key, socketChannel);
					synchronized (m_changeRequests) {
						debug(MyLogger.VERBOSE, "write() err ");
	...
	public String toString() {
		...
		String s = "tmsselector.rv1@" + this.hashCode();
	 ...
	 * @throws IOException
	 */
	public SocketChannel connect(String ip, int port, Object userobj) throws Exception {
		...
		synchronized (m_connecting) {
			...
			UserData item = new UserData(false, userobj, m_channelTimeoutSec);
	 ...
	 * @param timeoutSec
	 */
	public void waitForConnecting(SocketChannel ch, double timeoutSec) {
	...
	private void finishConnection(SelectionKey key) throws IOException {
		...
		try {
			...
			synchronized (m_changeRequests) {
				debug(MyLogger.VERBOSE, "finishConnnection() err ");
				this.m_changeRequests.add(new NioChangeRequest(channel, NioChangeRequest.REMOVE, 0));
			}
			debug(e);
	...
	void setChannelTimeoutSec(SocketChannel ch, double d) {
		UserData u = m_channels.get(ch);
	...
	double getChannelTimeoutSec(SocketChannel ch) {
	...
	public SocketChannel[] getChannels() {
		...
		synchronized (m_channels) {
			return m_channels.keySet().toArray(new SocketChannel[0]);
		}
