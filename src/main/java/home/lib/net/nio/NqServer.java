package home.lib.net.nio;

import java.nio.channels.SocketChannel;
import java.util.TimerTask;
import home.lib.lang.Timer2;
import home.lib.lang.UserException;
import home.lib.util.TimeUtil;

final public class NqServer implements INioConnectorListener, NqSelectorListener {

	private Object recvSync = new Object();

	private NqServer _this = this;

	private NqFactory m_bootstrap = null;

	// private AsynchronousServerSocketChannel m_server = null;

	private NqSelector m_selector = null;

	// ArrayList<MqClient> m_channels = new ArrayList<MqClient>();

	// Map<MqClient, TimeUtil> m_recvTime = new HashMap<MqClient, TimeUtil>();

	// private boolean m_alive = false;

	private INioServerListener m_inter = null;

	// private boolean m_isClosed = false;

	private Timer2 m_tmrListen = null;

	private String m_bindIp = "";

	private int m_port = 0;

	/**
	 * 
	 * @param bs
	 */
	public NqServer(NqFactory bs) {
		m_bootstrap = bs;

	}

	/**
	 * 
	 * 
	 * 
	 * 
	 * @param bindIp
	 * @param port
	 * @param in
	 * @return
	 * @throws Exception
	 */
	public NqServer select(String bindIp, int port, INioServerListener in) throws Exception {

		if (isAlive()) {
			throw new UserException("server(port:%d) is alive", port);
		}

		m_bindIp = bindIp;
		m_port = port;

		m_inter = in;

		m_tmrListen = new Timer2().schedule(new TimerTask() {
			public void run() {

				// if (m_isClosed == false) {
				try {
					// m_alive = true;

					m_selector = new NqSelector(_this);

					m_selector.bind(bindIp, port).get();

				} catch (Exception e) {
					e.printStackTrace();
				} finally {
					// m_alive = false;

					try {
						m_selector.close();
					} catch (Exception e) {
						e.printStackTrace();
					}
					m_selector = null;
				}

				m_bootstrap.cleanUp();
				TimeUtil.sleep(1000);
				// } // isclosed?

			}
		}, 100 );//do it one time!

		return this;
	}

	/**
	 * 
	 * 
	 * @param d
	 * @return
	 */
	public boolean waitBind(double d) {
		TimeUtil t = new TimeUtil();

		while (t.end_sec() < d && isAlive() == false) {
			TimeUtil.sleep(10);
		}

		return isAlive();
	}

	/**
	 * 
	 * @param ch
	 */
	public void channelClose(SocketChannel ch) {

		if (m_selector != null) {
			m_selector.channelClose(ch);
		} else {

			try {
				ch.close();
			} catch (Exception e) {

			}
		}

	}

	// /**
	// *
	// *
	// *
	// */
	// private void startTimer() {
	//
	// final Timer2 tmr = new Timer2();
	// tmr.schedule(new TimerTask() {
	// public void run() {
	//
	// if (isAlive() == false) {
	// tmr.cancel();
	// return;
	// }
	//
	// synchronized (m_channels) {
	// try {
	//
	// for (MqClient ch : m_channels) {
	//
	// // UserObj uo = (UserObj) ch.getUserObject();/
	// TimeUtil tu = m_recvTime.get(ch);
	//
	// if (tu != null) {
	// if (tu.end_sec() > m_dReceiveTimeoutSec && m_dReceiveTimeoutSec != 0) {
	// tu.start();
	// ch.close();
	// }
	// }
	// } // for
	//
	// } catch (Exception e) {
	// e.printStackTrace();
	// }
	// } // sync
	//
	// }
	// }, 1000, 1000);
	// }

	/**
	 * 
	 * 
	 */
	public void close() {
		// if (isAlive() == false)
		// return;
		// m_isClosed = true;

		// if (m_server != null) {
		// try {
		// m_server.close();
		// } catch (Exception e) {
		//
		// }
		// } // if

		if (m_selector != null) {
			try {
				m_selector.close();
			} catch (Exception e) {

			}
			m_selector = null;

		}

		if (m_tmrListen != null) {
			try {

				m_tmrListen.cancel();

			} catch (Exception e) {

			}
		}

		// synchronized (m_channels) {
		// for (int h = 0; h < m_channels.size(); h++) {
		//
		// try {
		// m_channels.get(h).close();
		// } catch (Exception e) {
		//
		// }
		// } // for
		//
		// m_channels.clear();
		// }
		// m_server = null;
		m_tmrListen = null;
	}

	// public boolean isClosed() {
	// return m_isClosed;
	// }
	// /**
	// */
	//
	// public int channelCount() {
	// return m_channels.size();
	// }
	//
	// /**
	// *
	// * @return
	// */
	// public MqClient[] getChannels() {
	// return m_channels.toArray(new MqClient[0]);
	// }

	/**
	 * 
	 * 
	 * 
	 * @return
	 */
	public boolean isAlive() {
		if (m_selector == null)
			return false;

		return m_selector.isAlive();
	}

	@Override
	public void connected(NqConnector sk) {
		// synchronized (m_channels) {
		// m_channels.add(sk);
		// // m_recvTime.put(sk, new TimeUtil());
		// } // sync

		m_inter.connected(this, sk);

	}

	@Override
	public void disconnected(NqConnector sk) {

		// synchronized (m_channels) {
		// m_channels.remove(sk);
		// // m_recvTime.remove(sk);
		// } // sync
		m_inter.disconnected(this, sk);

	}

	@Override
	public NqItem received(NqConnector ask, NqItem m, byte[] dataOfItem) {

		synchronized (recvSync) {

			NqItem rs = m_inter.recv(this, ask, m, m.data);
			return rs;

		} // sync0
	}

	// /**
	// *
	// *
	// * @param m
	// */
	// public void doRecv(MqItem m) {
	//
	//
	//
	//
	//
	// }

	/**
	 * 
	 * 
	 * @return
	 */
	public String getLocalAddress() {
		if (isAlive() == false)
			return null;

		return m_bindIp;
		// try {
		// //return m_server.getLocalAddress().toString();
		//
		// } catch (IOException e) {
		// // TODO Auto-generated catch block
		// e.printStackTrace();
		// }
		//
		// return null;

	}

	/**
	 * 
	 * 
	 * @param s
	 */
	public void debug(String s, Object... args) {

		// System.out.println("mqserver [" + String.format(s, args));

		m_bootstrap.debug(this, s, args);
	}

	@Override
	public void sendSucceeded(NqConnector ask, NqItem e) {
		m_inter.sendSucceeded(this, ask, e);

	}

	@Override
	public void sendFailed(NqConnector ask, NqItem e) {
		m_inter.sendFailed(this, ask, e);

	}

	@Override
	public String toString() {

		String s = "mqserver@" + this.hashCode();
		s += String.format("/%s:%d", m_bindIp, m_port);

		if (m_selector != null) {
			s += "+";
			s += m_selector.toString();
		}
		return s;

	}

	@Override
	public boolean putPath(NqConnector ask, String name) {

		return m_inter.putPath(ask, name);
		// return true;//ok put it!
	}

	@Override
	public NqChannelListener accepted(NqSelector svr, SocketChannel ask) {

		try {
			NqConnector cli = new NqConnector(m_bootstrap, m_selector);

			return cli.accept(m_selector, ask, this);

		} catch (Exception e) {

			e.printStackTrace();

		}
		return null;
	}

	@Override
	public boolean login(NqConnector nqConnector, String id, String pwd) {

		return m_inter.login(this, nqConnector, id, pwd);
	}

	@Override
	public void returnError(NqConnector nqConnector, UserException userException) {

		m_inter.returnError(this, nqConnector, userException);

	}
}
